#!/usr/bin/env python3
"""
BlueLotus V2 database initializer for macOS deployments.
...
def main() -> int:
    default_root = str(Path.home() / "bluelotus2")
    ap = argparse.ArgumentParser(description="Initialize BlueLotus V2 MySQL database.")
    ap.add_argument("--root", default=default_root, help="Installed BlueLotus root.")
    ...
    schema = Path(args.schema).expanduser() if args.schema else Path(__file__).resolve().parents[1] / "schema" / "bluelotus2_schema_mysql_8_4_9.sql"

    if not schema.exists():
        print(f"[FAIL] Schema file not found: {schema}")
        return 2

    admin_user = args.admin_user or input("MySQL admin user [root]: ").strip() or "root"
    admin_password = args.admin_password or getpass.getpass("MySQL admin password: ")
    ...
    statements = split_sql(schema.read_text(encoding="utf-8-sig"))
